    ...
    const columnDefinitions = columns
      .map(
        ({ column, dataType, constraints, defaultValue }) =>
          `${column} ${dataType} ${constraints ?? ""} ${defaultValue ? `DEFAULT ${defaultValue}` : ""}`
      )
      .join(", ");
...
// show tables
  ...
  export async function addColumn(tableName: string, values: Record<string,any>[]) {
    let noOfCols = 0;

    for(let i= 0 ; i<values.length ; i++){
        if(values[i].column && values[i].value)
            noOfCols++;
    }
    ...
    const columnDefinitions = values
      ...
      const largestArray = valuesArr.reduce((maxArr, currentArr) => 
    ...
    for(let i=0 ; i<largestArray.length ; i++){
        ...
        for(let j=0 ;j<noOfCols ; j++){
            ...
            if(typeof(valuesArr[j][i]) == "string")
                valuesString+= valuesArr[j][i] != 'DEFAULT'? "'"+valuesArr[j][i]+"'," :valuesArr[j][i]+',' ;
            else
                valuesString+= valuesArr[j][i] != 'DEFAULT'? valuesArr[j][i]+',' : valuesArr[j][i]+',';
        }
