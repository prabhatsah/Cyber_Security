...
export async function GET(req) {
    ...
    try {
        ...
        const command = `python "${theHarvesterPath}" -d ${query} -b all -l 50 -f output.json -n`;

        const { stdout, stderr } = await execPromise(command, { cwd: theHarvesterFolder });

        if (stderr) {
            console.error("Error:", stderr);
            return NextResponse.json({ error: "theHarvester execution failed", stderr }, { status: 500 });
        }

        // Read the output.json generated in the same directory
        const data = require("fs").readFileSync(path.join(theHarvesterFolder, "output.json"), "utf-8");
        ...
        return NextResponse.json({ error: "Failed to run theHarvester", details: error.message }, { status: 500 });
    }
}
