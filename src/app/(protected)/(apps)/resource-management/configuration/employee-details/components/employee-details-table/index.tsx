...
function EmployeeDataTable() {
  ...
  const [startInsEmp, setStartInsEmp] = useState<any>(false);

  const fetchEmpTableData = async () => {
    try {
      const softwareIdBaseApp = await getSoftwareIdByNameVersion("Base App", "1");

      console.log("software id ", softwareIdBaseApp);
      const empTableInstances = await getMyInstancesV2({
        softwareId: softwareIdBaseApp,
      ...
      const softwareIdSalesCRM = await getSoftwareIdByNameVersion("Sales CRM", "1");
      const uploadedFileInstances = await getMyInstancesV2({
        softwareId: softwareIdSalesCRM,
      ...
      const empMetadataInstances = await getMyInstancesV2({
        ...
        predefinedFilters: { taskName: "Edit Employee Details" },
      });
