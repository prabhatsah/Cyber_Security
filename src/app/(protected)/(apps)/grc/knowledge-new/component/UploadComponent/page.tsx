...
import { Download, FileText, Router, Upload } from "lucide-react";
...
import { startUploadData } from "./(backend-calls)";
import { startFrameworkData } from "../../../compliance/frameworks/(backend-calls)";
import { Label } from "@/shadcn/ui/label";
import { fetchFrameworkData } from "../../../compliance/frameworks/(backend-calls)/getData";
...
import { createUserMap } from "../../../governance/planning/page";
import {
  CreateUserMap,
  getUserDetailMap,
} from "../../../components/createUserMap";
import { toast } from "sonner";
...
export default function UploadComponent() {
  ...
  // }, 0);
  function transformToFrameworkStructure(controlsData, framework, owners) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [frameworks, users] = await Promise.all([
          fetchFrameworkData(),
          CreateUserMap(),
        ]);
