...
async function fetchUserMap(): Promise<{ value: string; label: string }[]> {
    try {
...
export default async function ParticularAduitDetails({ params }: { params: { auditId: string } }) {
    ...
    const isAllowedToScheduleMeeting = isCentralAdminUser || isAuditManagerUser || isCurrentUserAuditor;
    const isAllowedForFindingAndActions = isCentralAdminUser || isAuditManagerUser || isCurrentUserAuditor;
    return (
