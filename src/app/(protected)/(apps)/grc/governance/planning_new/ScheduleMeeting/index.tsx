...
// import { MultiSelect } from '../ModalForm/[auditId]/audit/AuditDropdown';
...
//   // function generateDescription(data: any) {
//   //   if (!data) return "";

//   //   const { policyName, controls = [] } = data;

//   //   const controlSections = controls.map((control: any) => {
//   //     const controlName = control.controlName;
//   //     const objNames = control.controlObjectives?.map((obj: any) => obj.name).join("\n\n") || "";

//   //     return `* Under ${controlName}, we will focus on:\n\n${objNames}`;
//   //   });

//   //   return `This meeting is scheduled to initiate and review the ${policyName} audit. We will be discussing key control categories including ${controls.map((c: any) => c.controlName).join(', ')}, along with their respective control objectives.\n\n${controlSections.join('\n\n')}\n\n`;
//   //   //Assigned team members will present updates, responsibilities, and readiness for each control area. Please come prepared with relevant documentation and status reports
//   // }
...
//   const selectedName = form.getValues("MEET_PARTICIPANTS");
//   // const filteredList = userIdNameMap.filter(item => selectedName.includes(item.value));
//   const filteredList = useMemo(() => {
//     return userIdNameMap.filter(item => selectedName.includes(item.value));
//   }, [userIdNameMap, selectedName]);
...
//                 </div>
//                 <div className="grid grid-cols-2 gap-3 mb-3">
//                   <FormInput formControl={form.control} name="MEETING_TITLE ooooooo" label="Title*" placeholder="Enter Meeting Title" />
...
//                     />
//                   </div> */}
//                   <FormMultiComboboxInput formControl={form.control} name="MEET_PARTICIPANTS" label="Participant(s)*" placeholder="Select Participants" items={userIdNameMap} defaultValue={form.getValues("MEET_PARTICIPANTS")} />
//                 </div>
//                 <div className="grid grid-cols-2 gap-3 mb-3">
...
// the code above is not working becuase of the changes done for multi select dropdown
...
// import FormMultiComboboxInput from '../ModalForm/[auditId]/audit/AuditDropdownFinal';
