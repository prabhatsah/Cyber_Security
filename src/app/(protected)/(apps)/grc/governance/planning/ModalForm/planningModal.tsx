...
export const PlanningFormSchema = z.object({
    AUDIT_NAME: z.string().min(1, { message: 'Please enter audit name.' }).trim(),
    AUDITEE_NAME: z.array(z.string()).min(1, { message: 'Please select auditee name.' }),
    AUDITOR_NAME: z.array(z.string()).min(1, { message: 'Please select auditor name.' }),
    ...
    SOP_TYPE: z.string().min(2, { message: 'Please select Framework.' }).trim(),
    FRAMEWORK_TYPE: z.string().min(2, { message: 'Please select Framework Type.' }).trim(),
    AUDIT_CYCLE: z.string().min(2, { message: 'Please select Audit cycle.' }).trim(),
...
export default function PlanningForm({ open, setOpen, userIdNameMap, editPlanning }:
    ...
    const [SOPType, setSOPType] = useState<{ value: string; label: string }[]>([]);
    ...
    const form = useForm<z.infer<typeof PlanningFormSchema>>({
        ...
        defaultValues: {
            ...
            AUDITEE_NAME: Array.isArray(editPlanning?.auditeeTeam) ? editPlanning.auditeeTeam : [], 
            AUDITOR_NAME: Array.isArray(editPlanning?.auditorTeam) ? editPlanning.auditorTeam : [], 
            ...
            SOP_TYPE: editPlanning ? editPlanning.policyName : '', 
            FRAMEWORK_TYPE: editPlanning ? editPlanning.framework : '', 
            AUDIT_CYCLE: editPlanning ? editPlanning.auditCycle : '',
            DESCRIPTION: editPlanning ? editPlanning.description : '', 
        ...
        if (buttonSelected == "NewAuditSaveAsDraft" || buttonSelected == "EditAuditSaveAsDraft") {
            if (planningModalTableRef.current) {
                ...
                const updatedControls = selectedFramework.controls
                    .filter((control: any) => controlsNewWeight.hasOwnProperty(control.controlName)) 
                    .map((control: any) => {
                        ...
                        const controlWeight = Number(controlsNewWeight[controlName] || control.controlWeight);

                        const updatedControlObjectives = control.controlObjectives
                            .filter((objective: any) => controlObjNewWeight[controlName]?.hasOwnProperty(objective.name)) 
                            .map((objective: any) => {
                                ...
                                return {
                                    ...objective,
                                    controlObjAssignee,
                                    controlObjweight: controlObjWeight, 
                                    ObjProgress: 0, 
                        ...
                        return {
                            ...
                            controlAssignee, 
                            controlWeight,
                            controlObjectives: updatedControlObjectives,
                            controlProgress: 0, 
