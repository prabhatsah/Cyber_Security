...
export default function UploadComponent({
  ...
  function transformToFrameworkStructure(controlsData) {
    ...
    for (const row of controlsData) {
      ...
      const nonConformityType = cleanString(rowData.nonconformity);
  ...
  const transformData = (jsonData: any[], labelValueMap: Record<string, string>) => {
    return jsonData.map((row) => {
      return Object.entries(row)
        .map(([field, value]) => {
          if (field.startsWith("_")) return null;
          // This is the new code to handle ownerfield
          if (field === "Owner Name") {
            return {
              field,
              value: labelValueMap[value as string] ?? value,
  ...
  const handleUpload = async () => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        // readHiddenSheets(workbook);
        const jsonData = XLSX.utils.sheet_to_json(firstSheet);
        const hiddenJsonData = XLSX.utils.sheet_to_json(secondSheet);
        // Transform the array into an object
        const labelValueMap = hiddenJsonData.reduce((acc: Record<string, string>, item: any) => {
          acc[item.label] = item.value;
          return acc;
        }, {} as Record<string, string>);
        // setUserIdNameMap(labelValueMap);
        const transformedData = transformData(jsonData, labelValueMap);
  ...
  // This is the code to read the hidden sheet
  const readHiddenSheets = (workbook: XLSX.WorkBook) => {
    workbook.SheetNames.forEach((sheetName, index) => {
      const sheetVisibility = workbook.Workbook?.Sheets?.[index]?.Hidden;

      if (sheetVisibility === 2) {
        console.log(`Sheet "${sheetName}" is hidden.`);
        const hiddenSheet = workbook.Sheets[sheetName];
        const jsonData = XLSX.utils.sheet_to_json(hiddenSheet);
        console.log(`Data from hidden sheet "${sheetName}":`, jsonData);
      } else {
        console.log(`Sheet "${sheetName}" is visible.`);
      }
    });
  };
