...
export default function AuditTable({
  ...
  const generateDummyAuditData = () => {
    const tagsOptions = ["Managerial", "Operational", "Technical"];
    const dummyData = [];
    ...
    for (let i = 1; i <= 5; i++) {
      const randomTags = Array.from(
        new Set(
          Array.from(
            { length: Math.floor(Math.random() * tagsOptions.length) + 1 },
      ...
      const auditData = {
        auditId: `AUDIT-${Date.now()}-${i}`, // Unique audit ID
        framework: `Framework-${i}`, // Framework ID
        frameworkType: i % 3 === 0 ? "Standard" : "BestPractice", // Framework type
        sopName: `SOP-${i}`, // SOP Name
        controlsSelected: [
          {
            controlId: `Control-${i}-1`,
            controlName: `Control Name ${i}-1`,
          },
          {
            controlId: `Control-${i}-2`,
            controlName: `Control Name ${i}-2`,
          },
        ], // Selected controls
        updatedOn: new Date().toISOString(), // Timestamp for the last update
        tags: randomTags, // Random tags
      };

      dummyData.push(auditData);
    }
  ...
  useEffect(() => {
    ...
    const dummyData = generateDummyAuditData();
  ...
  const columns: DTColumnsProps<Record<string, any>>[] = [
    ...
    {
      accessorKey: "controlsSelected",
      header: "Controls",
      cell: ({ row }) => (
        <div>
          {row.original.controlsSelected
            .map((control: any) => control.controlName)
            .join(", ")}
        </div>
      ),
    },
    {
      ...
      header: "Last Updated On",
      cell: ({ row }) => {
                      const dateValue = row.original.updatedOn
  ...
  const extraParams: DTExtraParamsProps = {
    ...
    defaultGroups: ["sopName"],
  };

  return <DataTable data={tableData} columns={columns} extraParams={extraParams} />;
}