...
const AddPolicyTable = ({
  open,
  setOpen,
  setExistingPolicyFormIdentifier,
}: {
  open: boolean;
  setOpen: (val: boolean) => void;
  setExistingPolicyFormIdentifier: any;
}) => {
  const router = useRouter();

  const form = useForm<FormData>({
    resolver: zodResolver(schema),
    defaultValues: { policyName: "", controls: [] },
    mode: "onSubmit",
  });

  const { control, handleSubmit } = form;

  const { setExistingPolicyForm } = useKBContext();
  ...
  const handleSubmitData = async (data: FormData) => {
    ...
    setExistingPolicyFormIdentifier(false);
    router.refresh();
  };
