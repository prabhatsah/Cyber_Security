...
import SecurityAIAgents from "@/components/SecurityAIAgents";
import {
  Chart as ChartJS,
  CategoryScale,
  LinearScale,
...
type VulnerabilityScan =
  Database["public"]["Tables"]["vulnerability_scans"]["Row"];
...
export default function Dashboard() {
  ...
  const { data: targets, isLoading: targetsLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("scan_targets").select("*");
  ...
  const findingsByDay =
    findings?.reduce((acc, finding) => {
      ...
      last7Days.forEach(({ date }) => {
        if (
          isWithinInterval(new Date(finding.created_at), {
    ...
    datasets: [
      {
        label: "Critical",
        data: last7Days.map(
          ({ label }) => findingsByDay.critical?.[label] || 0
        ),
  ...
  const completedScans =
    recentScans?.filter((scan) => scan.status === "completed").length || 0;
  const totalTargets = targets?.length || 0;
  const recentFindings =
    findings?.filter(
      (f) => differenceInHours(new Date(), new Date(f.created_at)) <= 24
    ).length || 0;
  ...
  const quickStats = [
    ...
    },
    {
      ...
      changeType:
        recentFindings > 0 ? ("increase" as const) : ("neutral" as const),
    },
    {
      name: "Security Score",
                    ...
                    <div className="flex items-center">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500">
                          {scan.scan_targets.target_url ||
                            scan.scan_targets.target_ip}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center">
...
// export default function Dashboard() {
//   return <h1 className="bg-white">Data coming soon...</h1>;
// }
