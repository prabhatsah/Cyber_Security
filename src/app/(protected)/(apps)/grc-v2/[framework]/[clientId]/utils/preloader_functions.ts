...
import {
  ...
  param,
  probleIdMapType,
  RoleMembershipInfoType,
  serviceIdWiseDetailsType,
} from "../discoveredDevices/types";
...
import { getUserMapForCurrentAccount } from "@/app/(protected)/(apps)/document-management/actions";
...
export async function getAllProbesDetails() {
  ...
  const onSuccess = async function (data) {
    ...
    const onSuccess = function (data) {
      ...
      for (
        let i = 0, probeCount = allProbeDetails.length;
        i < probeCount;
        i++
      ) {
        if (allProbeDetails[i].ACTIVE) {
          ref_2006.activeProbeDetailsMap[allProbeDetails[i].PROBE_ID] =
            allProbeDetails[i].PROBE_NAME;
        }
        //else{
        ref_2006.allProbeDetailsMap[allProbeDetails[i].PROBE_ID] =
          allProbeDetails[i].PROBE_NAME;
        //}
      }
  ...
  try {
    ...
    const resultingData = await getMyInstancesV2({
      softwareId,
      processName,
      predefinedFilters,
      projections,
      softwareId: baseSoftareId,
    });
...
export async function getMemoryCache(
  param: param,
  callback: callback
): Promise<memoryCacheReturn[] | unknown> {
  try {
    const softwareId = await getSoftwareIdByNameVersion("ITSM", "1");
...
export async function fetchCurrentRoleData(id: string | undefined) {
  ...
  const softwareId = id
    ? id
    : await getSoftwareIdByNameVersion(softwareName, version);
  const roleData = (await RoleMembershipInfo({
    userId: profile.USER_ID,
  })) as RoleMembershipInfoType[];

  const currentRole = roleData
    ? roleData.filter((role) => role.SOFTWARE_ID == softwareId)
    : [];
...
export function convertDate(dateStr_: string) {
  ...
  return (
    `${parsedDate.getFullYear()}-${pad(parsedDate.getMonth() + 1)}-${pad(
      parsedDate.getDate()
    )} ` +
    `${pad(parsedDate.getHours())}:${pad(parsedDate.getMinutes())}:${pad(
...
export async function getLatestStatus(param: getLatestStatus_paramType) {
  const onSuccess = async (
    data: InstanceV2Props<getLatestStatus_paramType>[]
  ) => {
    ...
    if (paramData) {
      ...
      for (const eachData of list) {
        ...
        for (const serviceName in monitoringData) {
          try {
            monitoringData[serviceName] = JSON.parse(
              monitoringData[serviceName]
            );
          } catch (e) {
        ...
        dashboardData.sort(
          (a, b) =>
            new Date(a.data_received_on).getTime() -
            new Date(b.data_received_on).getTime()
        );
      }
...
export async function liveStreamingDatafunc(
  params: getLatestStatus_paramType,
  serviceIdWiseDetails: serviceIdWiseDetailsType
) {
  const onSuccess = async (
    data: { metricsName: string; serviceId: string; value: unknown }[]
  ) => {
    console.log("data fetched: ", data);
