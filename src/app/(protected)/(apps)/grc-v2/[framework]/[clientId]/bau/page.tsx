...
export async function customeControlsData() {
  ...
  const customControlDatas = customControlsInstances.length > 0 ? customControlsInstances.map((customControlsInstance) => customControlsInstance.data) as { customControlId: string, refId: string, title: string }[] : []
...
export async function processFrameworksForDropdown(frameWorkId: string) {
  ...
  const frameworkData = frameworkInstance.length > 0 ? frameworkInstance[0].data : {};
...
export async function getBauTaskDetailsData(framework: string, clientId: string) {
  ...
  const bauConfigData = bauConfigInstances.length > 0 ? bauConfigInstances.map((bauInstance) => (bauInstance.data)) as EditFormSchema[] : [];

  const bauInstances = await getMyInstancesV2({
    processName: 'BAU',
    predefinedFilters: { taskName: "View BAU" },
    mongoWhereClause: `this.Data.frameworkId == '${framework}' && this.Data.clientId == '${clientId}'`
  });

  const bauData = bauInstances.length > 0 ? bauInstances.map((bauInstance) => (bauInstance.data)) as EditFormSchema[]  : [];
...
export async function getAllReferencedPolicyDetaisl() {
  ...
  const addConfigPolicyDatas = addConfigPolicyInstances.length > 0 ? addConfigPolicyInstances.map((policyInstace) => (policyInstace.data)) as referencedPolicies[] : null;
  const configReferencedPolicyMap = addConfigPolicyDatas?.map((addConfigPolicyData: referencedPolicies) => {
    return {
