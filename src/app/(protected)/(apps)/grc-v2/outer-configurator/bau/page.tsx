...
export interface BauFormSchema {
  taskId: string;
  task: string;
  details: string;
  objectiveIndex: string[];
...
export async function processControlsForDropdown() {
  ...
  for (const customControlData of customControlDatas) {
  ...
  return {dropDownControl,customControlDatas};
...
export async function getBauTaskDetailsData() {
  ...
  const bauData = bauInstances.length > 0 ? bauInstances.map((bauInstance) => (bauInstance.data)) : [];
  console.log(bauData);
  return bauData;
}
...
export async function getReferencedPolicyDetaisl() {
  ...
  const referencedPolicyMap = addPolicyDatas?.map((addPolicyData: referencedPolicies) => {
    return {
      value: addPolicyData.policyTaskId,
      label: addPolicyData.policyTitle
    }
  }) || []
  return referencedPolicyMap
...
export async function getTaskFrequencyType() {
  ...
  console.log(taskFrequencyDatas);
...
export default async function BAU() {
  ...
  // const dropDownControl = await createFrameworkControlsDropdown();
   const {dropDownControl,customControlDatas} = await processControlsForDropdown();
  ...
  return (
    <>
      <div className="mb-3 flex items-center justify-between">
        ...
        <BauOpenForm allUsers={allUsers} dropDownControl={dropDownControl} referencedPolicyMap={referencedPolicyMap} bauTaskFreqMap={bauTaskFreqMap} customControlDatas={customControlDatas}/>
      </div>
      <BauTab
        allUsers={allUsers}
