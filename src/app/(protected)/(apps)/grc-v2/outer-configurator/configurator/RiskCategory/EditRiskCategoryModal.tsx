...
export default function RiskCategoryEdit({ open, setOpen, riskCategoryData,profileData }: { open: boolean; setOpen: React.Dispatch<React.SetStateAction<boolean>>; riskCategoryData: Record<string, string>[]; profileData: Record<string, string>[]}) {
    ...
    useEffect(() => {
        ...
        const initialData = riskCategoryData.flatMap(obj =>
            obj.riskCategory
                ? Object.values(obj.riskCategory).map(entry => ({
                    ...entry,
                }))
                : []
        );
    ...
    async function saveImpactDataToBackend() {
        const instances = await getMyInstancesV2({
            ...
            predefinedFilters: { taskName: 'Edit Risk Category' },
        });
        ...
        const formattedData = categoryDataTableData.reduce((acc, curr) => {
            ...
            return acc;
        }, {} as Record<string, { riskCategoryId: string; riskCategory: string; addedOn: string; addedFrom: string }>);

        // Define newRiskCategoryAdded and riskId in the outer scope
        ...
        if (instances.length === 0) {
            ...
            const processId = await mapProcessName({ processName: "Metadata - Risk Category - Global Account" });
            ...
            await invokeAction({
                taskId,
                data: taskData,
                transitionName: 'update edit riskCategory',
