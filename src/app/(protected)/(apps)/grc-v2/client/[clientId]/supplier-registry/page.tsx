...
export async function getSupplierRegistryDatas(clientId: string) {
  ...
  const supllierRegistryDatas = supplierRegistryInstances.length > 0 ? supplierRegistryInstances.map((supplierRegistryInstance) => (supplierRegistryInstance.data)) : [];
...
export default async function SupplierRegistry({
  ...
  const supllierRegistryDatas = await getSupplierRegistryDatas(clientId) as SupplierRegistrySchema[];
  return (
    <>
      ...
      <div className="h-[92%] overflow-y-auto">
        <SupplierRegistryTable allUsers={allUsers} supllierRegistryDatas={supllierRegistryDatas} clientId={clientId}/>
      </div>
    </>
  )
}
