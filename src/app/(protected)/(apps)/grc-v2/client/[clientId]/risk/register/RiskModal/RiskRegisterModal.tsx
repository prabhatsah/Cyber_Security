...
export default function RiskRegisterForm({ open, setOpen, userIdNameMap, profileData, riskLibraryData, riskRegisterData, riskCategoryData, riskImpactData, editRisk, clientId }: { open: boolean; setOpen: React.Dispatch<React.SetStateAction<boolean>>; userIdNameMap: { value: string; label: string }[]; profileData: Record<string, any>; riskLibraryData: any[]; riskRegisterData: any[]; riskCategoryData: any[]; riskImpactData: any[]; editRisk: Record<string, any> | null; clientId: string }) { // editRisk type changed to Record<string, any> for flexibility
    const router = useRouter();
    debugger;
    ...
    const categoryType = riskCategoryData?.[0]?.riskCategory
    ...
    const impactType = Array.isArray(riskImpactData)
        ? riskImpactData.flatMap(obj =>
            obj.riskImpact
                ? Object.values(obj.riskImpact)
                    .filter((item: any) => item && item.riskImpact && (item.riskImpactId || item.riskImpact))
                    .map((item: any) => ({
                        value: item.riskImpactId?.toString() ?? item.riskImpact ?? "",
                        label: item.riskImpact ?? "",
                    }))
                : []
        )
    ...
    async function saveIncidentInfo(data: z.infer<typeof RegisterFormSchema>) {
        ...
        const riskRegisterPayload = {
            ...
            clientId: editRisk ? editRisk.clientId : clientId,
        };
        console.log("riskRegisterPayload to save", riskRegisterPayload);
