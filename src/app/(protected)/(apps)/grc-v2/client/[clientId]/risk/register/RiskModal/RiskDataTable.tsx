...
export default function RiskDataTable({ userIdNameMap, profileData, riskLibraryData, riskRegisterData, riskCategoryData, riskImpactData, clientId }: { userIdNameMap: { value: string; label: string }[]; profileData: Record<string, any>; riskLibraryData: { value: string; label: string }[]; riskRegisterData: Record<string, any>[]; riskCategoryData: Record<string, any>[]; riskImpactData: Record<string, any>[]; clientId: string }) {
  ...
  const columns: DTColumnsProps<Record<string, any>>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const categoryKey = row.original.riskCategory;
        if (
  ...
  return (
    <>
      ...
      {openRegisterForm && (
        <RiskRegisterForm open={openRegisterForm} setOpen={setOpenRegisterForm} userIdNameMap={userIdNameMap} profileData={profileData} riskLibraryData={riskLibraryData} riskRegisterData={riskRegisterData} riskCategoryData={riskCategoryData} riskImpactData={riskImpactData} editRisk={selectedRegister} clientId={clientId} />
      )}
    </>
  )
}
