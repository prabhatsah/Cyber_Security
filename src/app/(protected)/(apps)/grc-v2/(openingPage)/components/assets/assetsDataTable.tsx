...
export default function AssetsDataTable({ assetsData, userIdNameMap, profileData, currentSelectedClient }: { assetsData: Record<string, string>[]; userIdNameMap: { value: string, label: string }[]; profileData: string; currentSelectedClient: string | null }) {
    ...
    const columns: DTColumnsProps<Record<string, any>>[] = [
        ...
        },
        {
            ...
            header: "Owner",
            cell: ({ row }) => {
                const ownerId = row.original.assetOwner;
                const owner = userIdNameMap.find((user) => user.value === ownerId);
    ...
    const extraParams: DTExtraParamsProps = {
        ...
        extraTools: [
            profileData === currentSelectedClient && (
    ...
    return (
        <>
            ...
            <AssetsForm open={openAssetsForm} setOpen={setOpenAssetsForm} editAssetData={selectedAsset} userIdNameMap={userIdNameMap} profileData={profileData} />
        </>
    )
}
