...
export default async function BuildingsLayout({children,params}: {children: React.ReactNode; params: {id:string}}) {
  const buildingId = (await params).id
  return (
    <>
      {/* {children} */}
      <RenderAppBreadcrumb
          breadcrumb={{ level: 3, title: buildingId, href: `/bms/buildings/${buildingId}` }}
        />
      {children}
    </>
  );
