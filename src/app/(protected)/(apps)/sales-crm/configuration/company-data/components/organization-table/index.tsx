...
function OrganizationDataTable() {
    ...
    const fetchOrgTableData = async () => {
        try {
            const orgTableInstanceData = await getMyInstancesV2({
                processName: "HRMS - Organization",
                predefinedFilters: { taskName: "View" }
            });

            const instanceData = orgTableInstanceData[0]?.data as Record<string, { name: string; region: string; workingHours: string }> || {};
    ...
    const handleCloseModalEdit = () => {
        ...
        setSelectedId(null);
        fetchOrgTableData(); // Re-fetch data when modal closes
