...
import LeadEvent from "./eventCalender";

export default async function LeadEventTab({ params }: { params: { id: string } }) {
  const leadIdentifier = params.id; // Resolved value
  ...
  try {
    const eventDataInstances = await getMyInstancesV2<any>({
      ...
      mongoWhereClause: `this.Data.Created_User == "${userId}" && this.Data.leadsParentId == "${leadIdentifier}"`,
    });
  ...
  return (
    <>
      <LeadEvent leadIdentifier={leadIdentifier} eventData={eventData} />
    </>
  );
}
