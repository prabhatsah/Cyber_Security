...
export default function ResourceMainDataTable({ resourceData, userMaps, processedData, columnSchema, projectIdentifier }: { resourceData: any, userMaps: UserMaps; processedData: ResourceData[], columnSchema: ColumnSchema[], projectIdentifier: string }) {
    ...
    const extraParams: any = {
        ...
        grouping: true,
        defaultGroups: ["staff"],
        extraTools: [<IconButtonWithTooltip className='h-8' tooltipContent="Add/Edit Resource" onClick={() => { setIsModalOpen(true) }} ><Plus/></IconButtonWithTooltip>],
    };

    return (
        <>
            <DataTable columns={columnsProductDetails} data={processedData} extraParams={extraParams} />
            <ResourceModal isOpen={isModalOpen} onClose={handleCloseModal} projectIdentifier={projectIdentifier} resourceDataWithAllocation={resourceData} userMaps={userMaps}/>
        </>
    );
}