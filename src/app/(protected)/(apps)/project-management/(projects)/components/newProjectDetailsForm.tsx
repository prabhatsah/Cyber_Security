...
export default function NewProjectDetailsForm({ open, setOpen, projectmanager }: any) {
  ...
  async function saveProjectInfo(data: z.infer<typeof ProjectDetailsForm>) {
    console.log(data);
    console.log(checkedRows);
    const projectIdentifier = v4();
    const projectData = {
      projectIdentifier: projectIdentifier,
      projectStatus: "Project Created",
      projectManager: data?.MANAGER_NAME,
      projectTeam: checkedRows,
      projectName: data?.PROJECT_NAME,
      contractedStartDate: data?.START_DATE,
      contractedEndDate: data?.END_DATE,
      source: "Manual",
      productType: "Professional Service"
    }
    console.log(projectData);
    const processId = await mapProcessName({processName: "Project"});
    await startProcessV2({processId, data: projectData, processIdentifierFields: "projectIdentifier"});
    setOpen(false);
  }
