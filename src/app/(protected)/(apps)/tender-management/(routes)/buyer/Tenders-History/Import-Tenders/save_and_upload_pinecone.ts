...
export const tenderHistoryUpload = async (data: Record<string, any>) => {
  ...
  try {
    const handleUpload = async (uploadedFile: any, data: any) => {
    ...
    const processId = await mapProcessName({
      processName: "DM File Tender Upload Tracker",
    });
    await startProcessV2({
      processId,
      data: data,
      processIdentifierFields: "id,department",
    });
    await handleUpload(resourceData, data);
    console.log(
