...
import { ChatSectionResponse } from "../response-template-modal-components/chat-section";
...
const templateSchema = z.object({
  ...
  templateText: z.string().min(1, "Text is required"),
  selectedRows: z.array(z.string()).default([]),
});

type TemplateFormValues = z.infer<typeof templateSchema>;

export default function OpenResponseTemplateModal({
  isOpen,
  onClose,
  templateId,
}: TemplateProps) {
  ...
  const [rfpDataMap, setRfpDataMap] = useState<{ [key: string]: any }>({});
  ...
  async function getRfpData() {
    try {
      if (templateId) {
        const formData: any = await getTemplateData(templateId);
        Object.entries(formData).forEach(([key, value]) =>
          form.setValue(
            key as keyof TemplateFormValues,
            value as string | string[]
          )
        );

        setLoading(true);
        const rfpDataForTemplate = await uploadedResponseData();
        const rfpMap: { [key: string]: any } = {};
  ...
  const columns: DTColumnsProps<any>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-left">{row.original?.responseTitle || "N/A"}</div>
      ),
    },
    {
      accessorKey: "sector",
  ...
  const onSubmit: SubmitHandler<TemplateFormValues> = async (data) => {
    try {
      if (templateId) {
        ...
        const payload = { ...data, templateId: templateId };
        ...
        const payload = { ...data, templateId: uuid };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent
        className="max-w-[90dvw] h-[800px] overflow-y-hidden"
            ...
            <div className="grid col-span-1">
              <div className="w-full h-full">
                ...
                <div className="h-[650px]">
                  <ChatSectionResponse
                    onCopy={(text) => form.setValue("templateText", text)}
                  />
                </div>
              </div>
            </div>
