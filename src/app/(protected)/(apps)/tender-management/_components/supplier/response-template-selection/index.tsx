...
} from "../../../_utils/supplier/response-template-selection-functions";
...
const SelectCardModalResponse: React.FC<ModalProps> = ({
  ...
  useEffect(() => {
    ...
    const fetchCards = async () => {
      ...
      setError("");
      try {
        const published = await getPublishedDraftById(draftId);
        setDepartment(published.tender_department);
        setSector(published.sector);
        setProduct(published.product_service);
        const data: any[] = await getAllTemplates();
        console.log("Fetched Cards:", data);
        const filtered = data.filter(
          (temp) => temp.templateDepartment === published.tender_department
        );
        setCards(filtered);
