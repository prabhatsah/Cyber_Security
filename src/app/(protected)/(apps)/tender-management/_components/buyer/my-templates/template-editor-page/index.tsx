...
export default function TemplateEditor({ id }: TemplateEditorProps) {
  ...
  async function getRfpData() {
    try {
      if (id) {
        ...
        setTemplateData(formData.templateText);
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      router.replace(`/tender-management/buyer/my-templates`);
    }
    // startTransition(() => {
    //   router.refresh();
    // });
  ...
  const generateWithAI = async () => {
    ...
    const response = await fetch(link, {
      ...
      body: JSON.stringify({
        chatinput: `give me a tender template for a tender where tender type is ${selectedProduct}`,
      ...
      {loading ? (
        ...
        <div className="max-w-full mx-auto p-3 space-y-2">
          <div className="flex items-center gap-2">
            <Link href={`/tender-management/buyer/my-templates`}>
              <ArrowLeft />
            </Link>
          ...
          <div className="flex justify-end gap-2">
            ...
            <Sheet open={isChatOpen} onOpenChange={setIsChatOpen}>
              <SheetTrigger asChild>
                <Button variant="outline" onKeyDown={(e) => e.preventDefault()}>
                  Chat with AI{" "}
                </Button>
              </SheetTrigger>
              <SheetContent
