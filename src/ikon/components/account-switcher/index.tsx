...
export default function AccountSwitcher({
  ...
  accounts: AccountTreeProps[];
  activeAccountDetails: AccountTreeProps;
}) {
  const router = useRouter();
  const { isMobile } = useSidebar();
  //const [accounts, setAccounts] = useState<AccountTreeProps[]>([]);
  const [activeAccount, setActiveAccount] =
    useState<AccountTreeProps>(activeAccountDetails);
  ...
  const setSelectedAccount = async (account) => {
    setActiveAccount(account);
    await setActiveAccountId(account.ACCOUNT_ID);
    await revalidateData({ tags: ["subscribe-softwares"] });
    router.push("/");
  };
  // useEffect(() => {
  //     getAccountDetails();
            ...
            <Avatar className="h-8 w-8 rounded-lg">
              <AvatarImage
                src={
                  ...
                  (activeAccount?.ACCOUNT_NAME === "Keross"
                    ? "/assets/images/dark/symble-keross.png"
                    : "")
                }
                alt={activeAccount?.ACCOUNT_NAME}
              />
              <AvatarFallback className="rounded-lg">
                {activeAccount?.ACCOUNT_NAME?.substring(0, 1)}
              </AvatarFallback>
            </Avatar>
          </div>
