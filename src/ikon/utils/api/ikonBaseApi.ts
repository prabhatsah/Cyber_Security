...
async function ikonBaseApi({
  service,
  operation,
  arguments_,
  accountId,
  softwareId,
  isTicketRequried = true,
}: IkonBaseApiProps): Promise<any> {
  ...
  try {
    ...
    console.log(ikonErrorCode);

    if (ikonErrorCode == "AUTHENTICATIONEXCEPTION") {
      const message =
        "You are re-directed to Login Page. Possible reasons are:\n\t1) Your session has been idle for more than allowed timeout value.\n\t2) This is an unauthenticated access.";
        redirect("/login");
      //(await getLoggedInUserProfile()).USER_ID;
      console.log(message);

      return NextResponse.redirect(new URL("/login", "http://localhost:3333/IkonApps"));
    } else
